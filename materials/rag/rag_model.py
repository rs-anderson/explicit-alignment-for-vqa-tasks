...
class RagSequenceForGeneration(RagPreTrainedModel):
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if self.retriever is not None and context_input_ids is None:
            ...
            doc_ids = retriever_output['doc_ids']
            # set to correct device
            context_input_ids = context_input_ids.to(input_ids)
        ...
        for index in range(batch_size):
            ...
            # print(self.retriever.generator_tokenizer.batch_decode(generator_input_ids))
            # input('before!')
            ...
            # print(self.retriever.generator_tokenizer.batch_decode(output_sequences))
            # input('after!')
            if do_deduplication:
            ...
            # original way is is select top-k loss items
            # score[i] * p[y_i] 
            top_cand_inds = (-outputs["loss"]).topk(num_doc_return_sequences)[1]
            
            #############################
            # Let's try a majority vote!
            
            # answer_proposals = self.retriever.generator_tokenizer.batch_decode(output_sequences)
            # print('original', top_cand_inds, answer_proposals[top_cand_inds])
            # index_dict = {}
            # for index, p in enumerate(answer_proposals):
            #     index_dict.setdefault(p, index)
            # counter = Counter(answer_proposals)
            # print(counter)
            # top_proposals = [x[0] for x in counter.most_common(num_doc_return_sequences)]
            # print(top_proposals)
        ...
        return dict(
            generation_output=self._cat_and_pad(hypos, pad_token_id=self.config.generator.pad_token_id),
    ...
    def get_nll(
        ...
        # print(exclude_bos_score, use_bos)
        # input()
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if num_beams == 1:
            ...
            return dict(generation_output=self.greedy_search(
                input_ids,
            ...
            return dict(generation_output=self.beam_search(
                input_ids,
                beam_scorer,
                logits_processor=pre_processor,
                max_length=max_length,
