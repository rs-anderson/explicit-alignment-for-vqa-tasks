...
class Seq2SeqDataset(Dataset):
    def __init__(
        ...
        if type_path == 'train':
            csv_path = '/home/wl356/projects/Knowledge-based-visual-question-answering/data/ok-vqa/pre-extracted_features/passages/retriever_train.json'
        else:
            csv_path = '/home/wl356/projects/Knowledge-based-visual-question-answering/data/ok-vqa/pre-extracted_features/passages/retriever_test.json'
        
        with open(csv_path, 'r') as f:
            self.passage_annotations = json.load(f)
        
        self.passage_pos_items = {}
        for item in self.passage_annotations:
            question_id = item['question_id']
            pos_doc_ids = [p['id'] for p in item['ctxs']]
            self.passage_pos_items[question_id] = pos_doc_ids
    ...
    def __getitem__(self, index) -> Dict[str, torch.Tensor]:
        ...
        question_id, source_line = source_line.split('\t')
        question_id = int(question_id)

        passage_pos_items = self.passage_pos_items.get(question_id, [])

        source_line = self.prefix + source_line
        tgt_line = linecache.getline(str(self.tgt_file), index).rstrip("\n")
        split_tgt_line = tgt_line.split('\t')
        tgt_line = split_tgt_line[0]
        answers = split_tgt_line[1:]
        ...
        return {
            ...
            "answers": answers,
            'passage_pos_items': passage_pos_items,
        }
    ...
    def collate_fn(self, batch) -> Dict[str, torch.Tensor]:
        ...
        passage_pos_items = [x["passage_pos_items"] for x in batch]
