...
class T5ForConditionalGenerationWithVision(T5ForConditionalGeneration):
    ...
    def __init__(self, config, sys_config=None):
        ...
        self.mlp = MultiLayerPerceptron(input_dim=2048, 
                                        embed_dims=[config.d_model], 
                                        dropout=0)
    ...
    def set_system_config(self, sys_config):
        self.sys_config = sys_config
    ...
    def forward(
        ...
        text_feature_alignment_indice=None,
    ):
        if vision_input_features is not None:
            # Use MLP to extract vision features
            # print('vision_input_features', vision_input_features.shape)

            batch_size = vision_input_features.shape[0]
            num_tokens = vision_input_features.shape[1]
            dim = vision_input_features.shape[-1]
            vision_input_features = vision_input_features.reshape(-1, dim)
            # print('vision_input_features reshape', vision_input_features.shape)
            vision_input_embed = self.mlp(vision_input_features)
            # print('vision_input_embed', vision_input_embed.shape)
            vision_input_embed = vision_input_embed.reshape(batch_size, num_tokens, -1)
            # print('vision_input_embed reshape', vision_input_embed.shape)
            
            # embed_layer = get_input_embeddings() --> self.shared
            text_input_embed = self.shared(input_ids)
            # print('text_input_embed', text_input_embed.shape)
            inputs_embeds=torch.cat([vision_input_embed, text_input_embed], axis=1) # Cat along token axis
            input_ids = None  # set input ids to None to use inputs_embeds
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            ...
            if "encoder_outputs" not in model_kwargs: # or not isinstance(model_kwargs["encoder_outputs"], BaseModelOutputWithPastAndCrossAttentions):
