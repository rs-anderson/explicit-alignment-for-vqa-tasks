...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, latent_emb,
        ...
        """user aggregate"""
        # features of entities connected to this user are aggregated through interact_mat (mean_mat_list[0])
...
class GraphModel(nn.Module):
    def __init__(self, global_config, data_config, graph, adj_mat, pretrained_entity_emb=None):
        ...
        self._init_weight()
        #self.query_embed = torch.empty(self.n_users, self.emb_size) # not trainable
        if pretrained_entity_emb:
            self.entity_embed = pretrained_entity_emb
        self.entity_embed = nn.Parameter(self.entity_embed) # trainable entity embeddings
    ...
    def _init_weight(self):
        initializer = nn.init.xavier_uniform_
        #self.all_embed = torch.empty(self.n_entities, self.emb_size)
        self.entity_embed = initializer(torch.empty(self.n_entities, self.emb_size))
    ...
    def forward(self, batch=None):
        ...
        # n_users x hidden_size
        user_emb = torch.zeros(self.n_users, self.emb_size).to(user.device)
        # assign query embeddings to user_emb
        user_emb[user] = query_embeddings
        ...
        #print(u_e.shape, pos_e.shape, neg_e.shape)

        ################## in-batch negative sampling ###############
        batch_size = query_embeddings.shape[0]
        num_pos = 1
        num_neg = 1
        num_pos_and_neg = num_pos + num_neg
        # batch_size x dim  matmul  dim x (num_pos+num_neg)*batch_size  
        # -->  batch_size x (num_pos+num_neg)*batch_size
        in_batch_labels = torch.zeros(batch_size, batch_size*(num_pos+num_neg)).to(query_embeddings.device)
        step = num_pos_and_neg
        for i in range(batch_size):
            in_batch_labels[i, i] = 1
        # print('in_batch_labels', in_batch_labels)
        in_batch_labels = torch.argmax(in_batch_labels, dim=1)
        # print('in_batch_labels', in_batch_labels)

        in_batch_scores = torch.cat([torch.matmul(u_e, pos_e.T), torch.matmul(u_e, neg_e.T)], dim=1)

        loss = self.loss_fn(in_batch_scores, in_batch_labels)
        _, mf_loss, emb_loss, cor_loss = self.create_bpr_loss(u_e, pos_e, neg_e, cor)
        # loss, mf_loss, emb_loss, cor = self.create_CE_loss(u_e, pos_e, neg_e, cor)
        
        return loss+cor_loss, mf_loss, emb_loss, cor_loss


    def generate_query_embeddings(self, batch=None):
        user = batch['users']
        # print(user)
        # B x hidden_size
        query_embeddings = batch['query_embeddings']
    ...
    def load_pretrained_embeddings(self, pretrained_entity_emb):
        self.entity_embed.weight = pretrained_entity_emb.to(self.entity_embed.device)
        print('pre-trained embeddings are loaded!')
