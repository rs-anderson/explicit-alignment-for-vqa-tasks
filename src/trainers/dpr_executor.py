...
class DPRExecutor(BaseExecutor):
    ...
    def logging_results(self, log_dict, prefix='test'):
        ...
        metrics_to_log.update({
            f"retrieval_predictions_epoch{self.current_epoch}_MODE({self.config.mode})_SET({self.config.data_loader.use_dataset})": artifacts_to_log['test_table']
        })
        ...
        if self.config.mode == "test":
            ...
            json_path = os.path.join(self.config.results_path, '{}_predictions.json'.format(self.config.data_loader.use_dataset))
            with open(json_path, 'w') as json_f:
                json.dump(artifacts_to_log.to_write_data, json_f, indent=4)
