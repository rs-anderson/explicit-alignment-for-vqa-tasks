...
class ClipCapExecutor(BaseExecutor):
    ...
    def training_step(self, sample_batched, batch_idx):
        ...
        labels = sample_batched["input_ids"].detach().clone()
        ...
        for i, sample in enumerate(labels):
            ...
            for j, token in enumerate(sample):
                if token == -100:
                    labels[i, j] = self.tokenizer.pad_token_id
                    break
        ...
        train_batch = EasyDict(
            {
                ...
                "attention_mask": sample_batched["attention_mask"].to(
                    self.device
                ),
        ...
        forward_results = self.model(
            question_tokens=train_batch.input_ids,
            labels=train_batch.labels,
            prefix=train_batch.clip_embeddings,
            question_mask=train_batch.attention_mask,
